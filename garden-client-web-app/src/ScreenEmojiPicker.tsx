...
const ScreenEmojiPick: React.FC<ScreenEmojiPickingTypes> = () => {
  ...
  const showSelectedEmoji = (emojiData: EmojiData) =>
    setEmoji(emojiData.colons || defaultEmoji);
  const selectionMade = (emojiId: string) =>
    navigate(`/placer/:${encodeURIComponent(emojiId)}`);
